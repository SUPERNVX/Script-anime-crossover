--[[
    SCRIPT DE FARM (V0.6.2 - COMPLETO - CORRIGIDO)
]]
...
-- CONFIGURAÇÕES 
--====================================================================================
local TARGET_CFRAME = CFrame.new(67.91632843017578, 2.884401321411133, -6.006709098815918)
local UNIT_PLACEMENT_POSITION = Vector3.new(67.91632843017578, 2.884401321411133, -6.006709098815918)
...
local VICTORY_SCREEN_NAME = "EndOfGameGUI"
local REMOTE_PLAY_AGAIN_PATH = "RemoteEvents.UIRemoteEvents.PostEndGameClient"
local PLAY_AGAIN_ARGUMENT = "replay"
...
if not LocalPlayer then 
    error("FATAL: No LocalPlayer") 
end

local PlayerGui = LocalPlayer:WaitForChild("PlayerGui")
if not PlayerGui then 
    error("FATAL: No PlayerGui") 
end
...
-- FUNÇÕES AUXILIARES
--====================================================================================
local function customSplit(s, sep) 
    ...
    local ci = 1
    while true do 
        local si, ei = string.find(s, sep, ci, true)
        if not si then 
            table.insert(r, string.sub(s, ci))
            break 
        end
        table.insert(r, string.sub(s, ci, si-1))
        ci = ei + 1
        if ci > #s and si then 
            table.insert(r, "")
            break 
...
-- FUNÇÃO CORRIGIDA getRemote
local function getRemote(fp) 
    if remoteCache[fp] == false then 
        return nil 
    elseif remoteCache[fp] then 
        return remoteCache[fp] 
    end
    
    print("V0.6.2 getRemote: " .. tostring(fp))
    
    if type(fp) ~= "string" then 
        warn("getRemote: Path non-string")
        remoteCache[fp] = false
        return nil 
    end
    
    local pp = customSplit(fp, ".")
    if not pp or #pp == 0 then 
        warn("getRemote: customSplit fail")
        remoteCache[fp] = false
        return nil 
    end
    
    local co = ReplicatedStorage
    for _, pn in ipairs(pp) do 
        if co and co.FindFirstChild then 
            -- CORREÇÃO: Removendo o terceiro parâmetro incorreto
            local f = co:FindFirstChild(pn)
            if not f then 
                warn("getRemote: Part NF: " .. pn .. " in " .. co:GetFullName())
                remoteCache[fp] = false
                return nil 
            end
            co = f 
        else 
            warn("getRemote: Invalid CO for " .. pn)
            remoteCache[fp] = false
            return nil 
        end 
    end
    ...
    if co then 
        print("V0.6.2 getRemote: Found: " .. co:GetFullName() .. " (" .. co.ClassName .. ")")
        remoteCache[fp] = co 
    else 
        warn("V0.6.2 getRemote: Not Found: " .. fp)
        remoteCache[fp] = false 
    end
...
local function executeTeleport() 
    ...
    if s then 
        print("OK:TPsent.Pos:" .. tostring(humanoidRootPart.Position))
        return true 
    else 
        print("ERR:TPfail:" .. tostring(e))
        return false 
    end 
end
...
local function findUnitsFolder() 
    ...
    for _, n in ipairs(p) do 
        if c and c.WaitForChild then 
            c = c:WaitForChild(n, 10)
            if not c then 
                print("ERR:UFpart'" .. n .. "'NF")
                return false 
            end 
        else 
            print("ERR:InvalidObjUF")
            return false 
        end 
    end
    
    unitsFolderInstance = c
    if unitsFolderInstance then 
        print("OK:UF:" .. unitsFolderInstance:GetFullName())
        return true 
    else 
        print("ERR:UF NF")
        return false 
...
local function setGameSpeedTo2X() 
    print("V0.6.2 setGameSpeed")
    local r = getRemote(REMOTE_SPEED_CHANGE_PATH)
    
    if not r or not r.InvokeServer then 
        warn("Speed:RemoteInvalid")
        return 
    end
...
local function placeUnitOnMap() 
    print("V0.6.2 placeUnit")
    local r = getRemote(REMOTE_PLACE_UNIT_PATH)
    
    if not r or not r.InvokeServer then 
        warn("PlaceU:RemoteInvalid")
        return false 
    end
    
    print("PlaceU:Slot" .. UNIT_PLACEMENT_SLOT_NUMBER)
    local a = {
    ...
    if s then 
        print("PlaceU:CmdSent.Wait" .. DELAY_AFTER_PLACEMENT .. "s")
        wait(DELAY_AFTER_PLACEMENT)
        return true 
    else 
        warn("PlaceU:Fail:" .. tostring(e))
        return false 
    end 
end

local function findOrWaitForUnit() 
    print("V0.6.2 findUnit:'" .. UNIT_TO_FIND_NAME .. "'")
    
    if not unitsFolderInstance then 
        print("ERR:UFnil")
        return false 
    end
    ...
    for att = 1, MAX_UNIT_FIND_ATTEMPTS do 
        ...
        if targetUnitInstance and targetUnitInstance:IsA("Model") then 
            print("OK:Unit'" .. UNIT_TO_FIND_NAME .. "'found:" .. targetUnitInstance:GetFullName())
            return true 
        else 
            if targetUnitInstance then 
                print("WARN:Obj'" .. UNIT_TO_FIND_NAME .. "'foundNotModel:" .. targetUnitInstance.ClassName)
            else 
                print("INFO:Unit'" .. UNIT_TO_FIND_NAME .. "'NF.Att" .. att)
            end 
        end
        
        if att < MAX_UNIT_FIND_ATTEMPTS then 
            wait(CHECK_UNIT_INTERVAL)
        end 
    end
    
    print("ERR:Unit'" .. UNIT_TO_FIND_NAME .. "'NOTfound")
...
-- FUNÇÃO CORRIGIDA performSingleUpgrade
local function performSingleUpgrade(uM, uP)
    if not upgradeUnitRemoteFunc then 
        print("V0.6.2PSU:GetRemote")
        upgradeUnitRemoteFunc = getRemote(REMOTE_UPGRADE_UNIT_PATH)
        
        -- CORREÇÃO: Verificação mais robusta
        if not upgradeUnitRemoteFunc then
            warn("PSU:RemoteNotFound")
            return false
        end
        
        if not upgradeUnitRemoteFunc.InvokeServer then
            warn("PSU:RemoteHasNoInvokeServer")
            upgradeUnitRemoteFunc = nil
            return false
        end
        
        if type(upgradeUnitRemoteFunc.InvokeServer) ~= "function" then
            warn("PSU:InvokeServerNotFunction")
            upgradeUnitRemoteFunc = nil
            return false
        end
        
        print("V0.6.2PSU:RemoteCached")
    end
    ...
    print("V0.6.2PSU:Upg" .. uM.Name .. ",P" .. uP)
    
    local s, r = pcall(function()
        return upgradeUnitRemoteFunc:InvokeServer(uM, uP)
    end)
    
    if s then 
        if r == false then 
            print("V0.6.2PSU:UPG_NEGADO_SV " .. uM.Name .. ",P" .. uP)
            return false 
        end
        print("V0.6.2PSU:UPG_SUCESSO " .. uM.Name .. ",P" .. uP .. ".Res:" .. tostring(r))
        return true 
    else 
        warn("V0.6.2PSU:ERR_InvokeServer:" .. tostring(r))
        return false 
    end 
end
...
local function continuousUpgradeLoop()
    ...
    elseif not targetUnitInstance.Parent then
        ...
        if targetUnitInstance.Name then
             print("CUL END INFO: Nome da unidade: " .. targetUnitInstance.Name)
        else
             print("CUL END INFO: targetUnitInstance.Name não é acessível.")
        end
    else
        print("CUL END REASON: Condição do loop avaliada como false inesperadamente.")
        print("CUL END INFO: targetUnitInstance.Name: " .. (targetUnitInstance.Name or "Nome Indisponível"))
        print("CUL END INFO: Parent: " .. (targetUnitInstance.Parent and targetUnitInstance.Parent.Name or "nil"))
    ...
    if vS and vS.Enabled then 
        ...
        if pAR then 
            if pAR:IsA("RemoteEvent") then 
                ...
                local s, e = pcall(function()
                    pAR:FireServer(PLAY_AGAIN_ARGUMENT)
                end)
                
                if s then 
                    print("V0.6.2 checkPlayAgain:REfired!")
                    return true 
                else 
                    warn("V0.6.2 checkPlayAgain:ERRfireRE:" .. tostring(e))
                    return false 
                end 
            else 
                warn("V0.6.2 checkPlayAgain:ObjAt'" .. REMOTE_PLAY_AGAIN_PATH .. "'not RE.Is:" .. pAR.ClassName)
                return false 
            end 
        else 
            warn("V0.6.2 checkPlayAgain:RemotePlayAgain('" .. REMOTE_PLAY_AGAIN_PATH .. "')NF.")
            return false 
        end 
    else 
        return false 
...
local function masterGameLoop()
    ...
    while not stopAllLoops do
        ...
        if not initializePlayerAndCharacter() then 
            print("CRIT MLoop: P init fail. Stop.")
            stopAllLoops = true
            break 
        end
        
        if not executeTeleport() then 
            print("CRIT MLoop: TP fail. Stop.")
            stopAllLoops = true
            break 
        end
        ...
        if not placeUnitOnMap() then 
            print("CRIT MLoop: Place unit fail. Tentando nova rodada em 10s...")
            wait(10)
            continue 
        end
        ...
        if not findUnitsFolder() then 
            print("CRIT MLoop: UF fail. Tentando nova rodada em 10s...")
            wait(10)
            continue 
        end
        ...
        if findOrWaitForUnit() then 
            ...
            if not co_success then 
                warn("ERRO AO INICIAR CUL:", co_error)
                print("CRIT MLoop: Falha CUL. Tentando nova rodada 10s")
                wait(10)
                continue 
            end
            ...
            if stopAllLoops then 
                print("INFO MLoop: stopAllLoops ativado, saindo espera CUL.")
                break 
            end 
            ...
            while not victoryScreenPressed and not stopAllLoops and (tick() - waitStartTime < MAX_WAIT_FOR_VICTORY_SCREEN) do
                if checkForAndPressPlayAgain() then 
                    print("INFO MLoop: 'Reproduzir' ACIONADO.")
                    victoryScreenPressed = true 
                else 
                    print("INFO MLoop: Tela de vitória não detectada ou botão não pressionado. Esperando...")
                end
                wait(CHECK_FOR_VICTORY_SCREEN_INTERVAL)
            end

            if not victoryScreenPressed and not stopAllLoops then 
                print("WARN MLoop: Timeout tela de vitória ou botão não pressionado.") 
            elseif stopAllLoops then 
                print("INFO MLoop: stopAllLoops ativado durante espera tela vitória.")
                break 
            end
        else 
            print("CRIT MLoop: Unidade NÃO encontrada após colocação. Tentando nova rodada 10s...")
            wait(10) 
        end
